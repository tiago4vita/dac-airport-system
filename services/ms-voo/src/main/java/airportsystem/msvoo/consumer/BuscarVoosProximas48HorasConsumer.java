...
@Component
public class BuscarVoosProximas48HorasConsumer {
    ...
    @Autowired
    public BuscarVoosProximas48HorasConsumer(VooRepository vooRepository, AeroportoRepository aeroportoRepository, ObjectMapper objectMapper, RabbitTemplate rabbitTemplate) {
    ...
    @RabbitListener(queues = "voo.buscar-proximas-48-horas")
    public void receiveMessage(String msg) throws JsonMappingException, JsonProcessingException {
        Map<String, Object> response = new HashMap<>();
        try {
            if (msg == null || msg.isEmpty()) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                        "Mensagem vazia. A mensagem deve conter as datas de início e fim.");
            }

            JsonNode jsonNode = objectMapper.readTree(msg);

            // Expecting the start and end dates in the message
            LocalDate dataInicio = LocalDate.parse(jsonNode.get("data").asText());
            LocalDate dataFim = LocalDate.parse(jsonNode.get("data-fim").asText());

            // Fetch the Voos by data and data-fim
            List<Voo> voos = vooRepository.findByDataHoraBetween(dataInicio.atStartOfDay(), dataFim.atTime(23, 59, 59));
            if (voos.isEmpty()) {
                ...
                response.put("inicio", dataInicio.format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
                response.put("fim", dataFim.format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
