...
public class RealizarVooConsumer {
    ...
    @Autowired
    public RealizarVooConsumer(VooRepository vooRepository, AeroportoRepository aeroportoRepository, ObjectMapper objectMapper, RabbitTemplate rabbitTemplate) {
    ...
    @RabbitListener(queues = "voo.realizar")
    public void receiveMessage(String msg) throws JsonMappingException, JsonProcessingException {
        ...
        try {
            ...
            } else if (voo.getEstado() != "CONFIRMADO") { // Assuming 0 = CONFIRMADO
                response.put("success", false);
                response.put("message", "Voo não está com status CONFIRMADO.");
            } else {
                // Set status to REALIZADO (2)
                voo.setEstado(Voo.EstadoVoo.REALIZADO); // Assuming 2 = REALIZADO
                vooRepository.save(voo);

                response.put("success", true);
                response.put("message", "Voo realizado com sucesso.");
